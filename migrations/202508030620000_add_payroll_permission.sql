...
-- Assign payroll permission to Admin and HR Manager roles
DO $$
DECLARE
  admin_role_id UUID;
  hr_role_id UUID;
  payroll_perm_id UUID;
BEGIN
  -- Get role IDs
  SELECT id INTO admin_role_id FROM public.roles WHERE name = 'Admin';
  SELECT id INTO hr_role_id FROM public.roles WHERE name = 'HR Manager';
  SELECT id INTO payroll_perm_id FROM public.permissions WHERE name = 'Payroll Management';
  
  -- Assign to Admin role
  INSERT INTO public.role_permissions (role_id, permission_id)
  VALUES (admin_role_id, payroll_perm_id)
  ON CONFLICT (role_id, permission_id) DO NOTHING;
  
  -- Assign to HR Manager role
  INSERT INTO public.role_permissions (role_id, permission_id)
  VALUES (hr_role_id, payroll_perm_id)
  ON CONFLICT (role_id, permission_id) DO NOTHING;
END $$;