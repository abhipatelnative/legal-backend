-- Migration: Dynamic leave hours based on leave type config
-- Base: latest process_daily_attendance and backfill_attendance_counting function bodies
...
begin
    ...
    loop
        ...
        leave_allow_hourly := false;
        leave_max_hours := null;
        is_half_day := false;
        is_hourly_leave := false;

        /* -------------------------------
           step 1 : leave (highest priority)
           Dynamic rule: payable hourly leave with valid time window = present
        -------------------------------- */
        select
            true,
            lt.salary_payable,
        ...
        where es.contract_id = emp.contract_id
...
IS 'Dynamic hourly leave support: approved payable hourly leave with valid start/end time counts as present.';
