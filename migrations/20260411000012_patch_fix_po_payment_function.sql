-- Cash & Bank Integration - Patch: Fix record_po_payment Function
-- Migration: 20260411000012_patch
-- Purpose: Drop existing record_po_payment function(s) and recreate with bank_account_id parameter

-- Drop ALL existing versions of the function (PostgreSQL may have multiple overloads)
DO $$
BEGIN
  -- Try dropping with original signature (7 parameters)
  BEGIN
    DROP FUNCTION IF EXISTS public.record_po_payment(
      p_po_id UUID,
  ...
  -- Try dropping with 8 parameters (if already partially migrated)
  BEGIN
    DROP FUNCTION IF EXISTS public.record_po_payment(
      p_po_id UUID,
      p_amount DECIMAL(15,2),
...
-- Now create the new version with all 8 parameters
