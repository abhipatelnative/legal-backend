-- Cash & Bank Module - Fix Ledger Empty Balance
-- Migration: 20260416000005
-- Purpose: Always return an "Opening Balance" row as the first entry to avoid 0-balance displays on empty ranges

-- Drop existing to change logic
DROP FUNCTION IF EXISTS public.get_account_ledger(UUID, TIMESTAMP, TIMESTAMP) CASCADE;

-- Recreate with virtual opening balance row
...
BEGIN
  ...
  WITH transactions AS (
    ...
    SELECT
      ...
      '00000000-0000-0000-0000-000000000000'::UUID AS txn_id -- Dummy ID for stable sorting
...
COMMENT ON FUNCTION public.get_account_ledger IS 'Get account transaction history with a mandatory Opening Balance row as the first entry to ensure the ledger always shows account state.';
